...
class LDPC5GEncoder(Layer):
    ...
    def _encode_fast(self, s):
        ...
        # print('p_b_1',p_b_1.shape, tf.math.reduce_sum(p_b_1, [1]))
        p_b_2 = self._matmul_gather(self._pcm_c2_ind, p_a)
        # print('p_b_2',p_b_2.shape, tf.math.reduce_sum(p_b_2, [1]))
        p_b = p_b_1 + p_b_2
        # print('p_b',p_b.shape, tf.math.reduce_sum(p_b, [1]))

        c = tf.concat([s, p_a, p_b], 1)
        w = tf.concat([p_a, p_b], 1)
        # print('w',w.shape, tf.math.reduce_sum(w, [1]))
        w = tf.cast(w, tf.uint8)
        w = tf.bitwise.bitwise_and(w, tf.constant(1, tf.uint8))
        w = tf.cast(w, self.dtype)
        # print('w_mod',w.shape, tf.math.reduce_sum(w, [1]))
        # print('c',c.shape, tf.math.reduce_sum(c, [1]))
        ...
        # print('c_mod',c.shape, tf.math.reduce_sum(c, [1]))
        c = tf.expand_dims(c, axis=-1) # returns nx1 vector
        # print('c_ret',c.shape, tf.math.reduce_sum(c, [1,2]))
        # print('c_sho',c[:,2*self._z:].shape, tf.math.reduce_sum(c[:,2*self._z:], [1,2]))
        return c
    ...
    def call(self, inputs):
        ...
        # print('c_short',c_short.shape, tf.math.reduce_sum(c_short, [1]))
